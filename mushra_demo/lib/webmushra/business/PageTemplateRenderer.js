...
PageTemplateRenderer.prototype.renderNavigation = function(_parentId) {
  ...
  if (this.pageManager.getPageIndex() > 0 && this.showButtonPreviousPage) {
    var buttonPrevious = $("<button data-role='button' data-inline='true' onclick='" + this.pageManager.getPageManagerVariableName() + ".previousPage();'>⬅️ " + this.pageManager.getLocalizer().getFragment(this.language, "previousButton")  + "</button>");
  ...
  if (this.pageManager.getPageIndex() < (this.pageManager.getNumPages() - 1)) {
    var buttonNext = $("<button id='__button_next' data-role='button' data-inline='true' onclick='" + this.pageManager.getPageManagerVariableName() + ".nextPage();'>" + this.pageManager.getLocalizer().getFragment(this.language, "nextButton")  + " ➡️</button>");
